Shader "Zan/Unlit/MapLOD"
{
	Properties
	{
		[Caption(Base Map Properties)]
		_MainTex( "Base Map", 2D) = "white" {}
		_MainTex_LOD( "Base Map LOD", Range( 0, 7)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			UNITY_INSTANCING_BUFFER_START( Props)
				...
				UNITY_DEFINE_INSTANCED_PROP( int, _MainTex_LOD)
			...
			fixed4 frag( VertexOutput i) : COLOR
			{
				...
				fixed4 color = tex2D( _MainTex, float4( i.uv0, 0, UNITY_ACCESS_INSTANCED_PROP( Props, _MainTex_LOD)));
			#if !defined(_VERTEXCOLORBLENDOP_NONE) || !defined(_VERTEXALPHABLENDOP_NONE)
